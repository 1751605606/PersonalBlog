...
* the values passed for the limit/offset are sent as bound parameters.   Some
  models have observed that Oracle produces a poor query plan when the values
...
Some models have reported better performance when the entirely different
approach of a window query is used, i.e. ROW_NUMBER() OVER (ORDER BY), to
provide LIMIT/OFFSET (note that the majority of models don't observe this).
    ...
    def _prepare_reflection_args(
        ...
        **kw
        ...
        if dblink:
            ...
            # will need to hear from more models if we are doing
