...
@blue_print.route('/api/user/register', methods=['POST'])
def user_register():
    if request.method == 'POST':
        ...
        if not user:
            if username == "admin":
                group_id = 1
            user = User(username, password, group_id)
            try:
                user.add_user()
            # 发生异常
...
@blue_print.route('/api/user/login', methods=['POST'])
def user_login():
    if request.method == 'POST':
        json_data = json.loads(request.get_data())
        username = json_data.get("username")
...
@blue_print.route('/api/user/modify', methods=['POST'])
def user_modify():
    if request.method == 'POST':
